...
Eseguito automaticamente all'avvio del backend (vedi server.py startup hook):
...
L'operazione è idempotente: usa INSERT ... ON DUPLICATE KEY UPDATE, quindi può
essere eseguita anche su un DB già popolato senza causare errori o duplicati.
"""
...
# Percorso assoluto: backend/src/init_db.py -> .. -> .. -> /progetto/gs_data
...
def populate_from_gs_data() -> dict:
    ...
    for path in _iter_gs_files(GS_DIR):
        ...
        for entry in entries:
            ...
            # Calcola il dominio: la entry può averlo già, altrimenti lo deriviamo dall'URL
